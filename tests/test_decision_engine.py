"""Tests for app.decision.engine (unit tests; no LLM calls)."""
...
def test_prepare_groups_meal_daily_totals():
    ...
    groups_data, save_data = engine._prepare_groups(bills_map)
    assert len(groups_data) == 2  # two dates
    daily_totals = {g["date"]: g["daily_total"] for g in groups_data}
    assert daily_totals["10/01/2025"] == 150
    assert daily_totals["11/01/2025"] == 80
    assert all(g["category"] == "meal" for g in groups_data)
...
def test_prepare_groups_unknown_category_treated_as_non_meal():
    ...
    groups_data, _ = engine._prepare_groups(bills_map)
    assert len(groups_data) == 1
